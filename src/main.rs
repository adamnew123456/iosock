...
use std::io::{self, pipe, stdout, PipeReader, Read, Write};
...
impl<T: Default + Copy, const N: usize> FlipBuffer<T, N> {
    ...
    pub fn with_read<E, F: FnOnce(&[T]) -> Result<usize, E>>(
        &mut self,
        func: F,
    ) -> Result<usize, E> {
        ...
        match func(slice) {
            Ok(was_read) => {
                if was_read > readable {
                    panic!(
                        "Readable range is {}..{} ({} bytes) but {} was consumed",
                        self.read_cursor, self.read_max, readable, was_read
                    );
                }
    ...
    /// slice, this function panics.
    pub fn with_write<E, F: FnOnce(&mut [T]) -> Result<usize, E>>(
        &mut self,
        func: F,
    ) -> Result<usize, E> {
        ...
        match func(slice) {
            Ok(was_written) => {
                if was_written > writable {
                    panic!(
                        "Writable range is {}..{} ({} bytes) but {} was written",
                        self.write_cursor, N, writable, was_written
                    );
                }
